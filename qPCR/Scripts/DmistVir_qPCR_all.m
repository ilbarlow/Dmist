...
fname = 'C:\Users\ilbar\Documents\MATLAB\DreammistPaper\qPCR\Data\DmistVir\210617\dmist_vir_qPCR.xlsx';
uiopen(fname,1)
...
for p=1:3
    xlswrite(fullfile(fileparts(fileparts(fname)), 'VirqPCRStats.xls'), Pt{p}, genenames{p});
end
...
for p=1:size(all,2)
    ...
    text(1,1.5, strcat('anova1, p=', num2str(P{p})));
...
box off
legend('Dmist, n=3')
text(1,1.5, strcat('anova1, p=', num2str(P{1})));
print(fullfile(fileparts(fileparts(fname)),'Figures', 'DmistqPCR'), '-depsc', '-tiff')
savefig(fullfile(fileparts(fileparts(fname)),'Figures', 'DmistqPCR.fig'))
saveas(gcf, fullfile(fileparts(fileparts(fname)),'Figures', 'DmistqPCR'), 'tiff')