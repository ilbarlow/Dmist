function StateTransitions (folder, saveDir,names, ControlPos, cmap, ymaxD, ymaxN)
...
%Inputs:
    ...
    %ymaxD - maximum y values for Day state transition plots
    
    %ymaxN - maximum y values for Night state transition plots
...
%need to reassign the clusters so that 1=low activity and 5 = high
    ...
    dist_plot(stateAllD, size(pd2,2), cmap, names, pd2, fullfile(saveDir, 'dayTransitions'), ymaxD)
    dist_plot(stateAllN, size(pn2,2), cmap, names, pn2, fullfile(saveDir, 'nightTransitions'),ymaxN)
    ...
    for i = 1:size(stateAllD,1)
        ...
        heatDm(i,:) = nanmean(stateAllD{i,3},1);
        heatNm(i,:) = nanmean(stateAllN{3,3},1);
    end
    
    %now interweave for heatplot to create a 1 x 10 matrix for imagesc with
    %WT, Het and Hom next to each other
    heatD2 = NaN(1,nConditions*(K(1)+1));
    heatN2 = NaN(1,nConditions*(K(1)+1));
    for j = 1:size(heatDmed, 2)
        temp = heatDmed(:,1)'; %initialise
        temp2 = heatNmed(:,1)';
        heatD2 = temp(:,ones(nConditions,1)*(1:size(temp,2))); %double original matrix
        heatN2 = temp2(:,ones(nConditions,1)*(1:size(temp,2)));
        for j = 2:nConditions %loop through to fill
            heatD2(:,j:nConditions:end) = heatDmed(:,j)';
            heatN2(:,j:nConditions:end) = heatNmed(:,j)';
        end
        clear temp temp2
    end

    heat = vertcat(heatD2, heatN2);
    %custom colorscheme
    ...
    %lim = caxis;
    caxis([0 0.4])
    set(gca, 'XTick', [round(nConditions/2):nConditions:size(heat,2)], 'XTickLabel',...
        [1 2 3 4 5], 'YTick', [1 2], 'YTickLabel', {'Day' 'Night'})
    colorbar;
    rn = find(pn2(3,:)<0.05);
    rd = find(pd2(3,:)<0.05);
    if isempty(rn) == 0
        for i =1:size(rn,2)
            text((rn(i) * nConditions)-1,2, num2str(round(pn2(3,rn(i)),3)),...
                'HorizontalAlignment', 'center');         
        end
    end
    if isempty(rd) == 0
        for i =1:size(rd,1)
            text((rd(i) * nConditions)-1, 1, num2str(round(pd2(3,rd(i)),3)),...
                'HorizontalAlignment', 'center'); 
        end
    end
    box off
    savefig(fullfile(saveDir, 'HeatMap.fig'));
    print(fullfile(saveDir, 'HeatMap'), '-dsvg');
    print(fullfile(saveDir, 'HeatMap'), '-dtiff');
    ...
    %preallocate arrays
    %transition matrices
    transitionN = cell(size(sleepStructure,2), size(fishStatesN,2), size(fishStatesN,3), K(2)+1); %exps, genotypes, days, states
    transitionD = cell(size(sleepStructure,2), size(fishStatesD,2), size(fishStatesD,3), K(1)+1);
    
    %squeezed transition matrices
    transitionN2 = cell(size(sleepStructure,2), size(fishStatesN,2), size(fishStatesN,3)); %exps, genotypes, days
    ...
    concatTransitionPlotN = cell(size(transitionN2,3),1); %each day
    concatTransitionPlotD = cell(size(transitionD2,3),1);
    for d = 1:size(concatTransitionPlotN,1)
       concatTransitionPlotN{d} = scrap(:,((n+1)*d)-(d+(n-1)):((n+1)*d)-d,:);
       concatTransitionPlotD{d} = temp(:,((n+1)*d) - (d+(n-1)):((n+1)*d)-d,:);
    end
    clear scrap temp
    ...
    allTransNmed = cell(size(concatTransitionPlotN,1),1);
    allTransDmed = cell(size(concatTransitionPlotD,1),1);
    allTransNmean = cell(size(concatTransitionPlotN,1),1);
    allTransDmean = cell(size(concatTransitionPlotD,1),1);
    for d = 1:size(allTransDmed,1)
        allTransNmed{d}=squeeze(nanmedian(concatTransitionPlotN{d}(:,:,:)))';
        allTransDmed{d}=squeeze(nanmedian(concatTransitionPlotD{d}(:,:,:)))';
        allTransNmean{d} = squeeze(nanmean(concatTransitionPlotN{d}(:,:,:)))';
        allTransDmean{d} = squeeze(nanmean(concatTransitionPlotD{d}(:,:,:)))';
    end
    
    %now export these as spreadsheets
    Days = {'Day1' 'Day2' 'Day3'}
    for d = 1:size(allTransDmean,1)
        xlswrite(fullfile(saveDir, 'DayTransitionsMedian.xls'), allTransDmed{d}, Days{d});
        xlswrite(fullfile(saveDir, 'NightTransitionsMedian.xls'), allTransNmed{d}, Days{d});
        xlswrite(fullfile(saveDir, 'DayTransitionsMean.xls'), allTransDmean{d}, Days{d});
        xlswrite(fullfile(saveDir, 'NightTransitionsMean.xls'), allTransNmean{d}, Days{d});
    end
