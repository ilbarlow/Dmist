%Generate sleep wake traces and distribution plots for all dmist i8
%experiments
...
cmap = [1 0 1; 0 1 0; 0 0 0];  %chose the red, blue and black part of the colormap
...
%    title(strcat('Average Activity', sleepStructure(j).name), 'Fontsize', 20)
  ...
  saveas(gcf, fullfile(fileparts(folder), 'expFigures', strcat(sleepStructure(j).name(1:6),...
...
for i = 1:size(sleepStructure,2) %each experiment
    ...
    savefig(fullfile(fileparts(folder), 'expFigures', strcat(sleepStructure(i).name(1:6),...
        'day_sleep.fig')))
    print(fullfile(fileparts(folder), 'expFigures', strcat(sleepStructure(i).name(1:6),...
        'day_sleep')), '-depsc')
    saveas(gcf,fullfile(fileparts(folder), 'expFigures', strcat(sleepStructure(i).name(1:6),...
        'day_sleep')), 'tiff')
   ...
   rectangle('Position', [0 0 4 2000], 'Facecolor', [0.9 0.9 0.9],...
    ...
    subplot(1,3,3);
    rectangle('Position', [0 0 4 50], 'Facecolor', [0.9 0.9 0.9],...
       'Edgecolor', [1 1 1]);
    hold on
...
for e=1:size(sleepStructure,2)
    saveDir = fullfile(fileparts(folder), strcat(sleepStructure(e).name(1:6), 'sleep_stats_i8Exps.xls'));
